# Payments Challenge — Go

Go implementation of the two-service payment system described in the
[root README](../README.md).

**Stack:** Go 1.22 + Gin + mongo-driver v2 + Sarama consumer + kafka-go producer + zerolog
...
```text
services/<service>/
├── main.go            # entry point: wiring, HTTP server, Kafka
├── api/rest/          # handlers.go + routes.go
...
To run a service outside Docker you need Go 1.22 plus MongoDB and Kafka reachable at
`MONGODB_URI` and `KAFKA_BROKERS` (see `docker-compose.yml` for the values used in Compose):

```bash
cd services/payments-api && go run .
```
